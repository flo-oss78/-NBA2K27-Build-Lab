...
function optimizeBuild(){
 ...
 const content=`<div class="modal-kicker">OPTIMISEUR V20.4 • ${style.toUpperCase()}</div><h2>Optimisation terminée</h2><p class="sub">Le moteur a déplacé des points d’un attribut à l’autre sans en ajouter. L’équivalence entre attributs et les caps utilisés restent <strong>indicatifs</strong> tant que les tables internes complètes de 2K27 ne sont pas publiques.</p><div class="modal-stats"><div><b>${before.unlocked}</b><span>badges avant</span></div><div><b>${after.unlocked}</b><span>badges après</span></div><div><b>${changed.length}</b><span>attributs modifiés</span></div></div><h3>Principaux changements</h3><div class="modal-attrs">${top.length?top.map(x=>`<div><span>${x.name}</span><b>${x.from} → ${x.to}</b></div>`).join(''):'<div><span>Build déjà optimisé</span><b>Aucun changement</b></div>'}</div><div class="modal-actions"><button data-close-modal>Garder ce build</button><button class="secondary" data-close-modal>Fermer</button></div>`;
...
/* Le bouton n'existe que sur la page du builder. */
document.getElementById('optimize')?.addEventListener('click',optimizeBuild);
